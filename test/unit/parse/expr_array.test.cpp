...
TEST_CASE_WITH_FIXTURE("Parse::Array: Empty",
                       "[Unit][Parse][Expr]") {
...
TEST_CASE_WITH_FIXTURE("Parse::Array: 3 arguments",
                       "[Unit][Parse][Expr]") {
    assertExprWithParams({
        .source =
        "[1,x,2.1]",
...
TEST_CASE_WITH_FIXTURE(
    ...
    assertExprWithParams({
        ...
        .expectedErrors = {
            {.context = Context::Array, .reason = Reason::MissingExpr},
        },
    });
}
...
TEST_CASE_WITH_FIXTURE(
    ...
    assertExprWithParams({
        ...
        .expectedErrors = {
            {.context = Context::Array, .reason = Reason::MissingEnclosingSymbol},
        },
    });
}
