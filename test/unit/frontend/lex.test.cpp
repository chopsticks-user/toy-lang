...
protected:
  auto lex(Str source) -> void {
    ...
    m_tokens = tlc::apply<tlc::lex::Lexer>(std::move(iss));
  }

  auto nTokens() const -> sz {
  ...
  auto tokens() const -> const Vec<Token> & {
    return m_tokens;
  }
...
#define REQUIRE_TOKENS(...) \
  do { \
    tlc::Vec<Token> expected{__VA_ARGS__}; \
    auto actual = tokens(); \
