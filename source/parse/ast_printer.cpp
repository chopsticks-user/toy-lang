...
export module parse:ast_printer;

import syntax;

namespace tlc::parse {
    export class ASTPrinter final : public syntax::SyntaxTreeVisitor<Str> {
    ...
    public:
        ...
        constexpr auto operator()(syntax::type::GenericArguments const& node) -> Str {
            return std::format(
                ...
                node.line(), node.column(), node.size(), this->visitChildren(node)
            );
        }
        ...
        constexpr auto operator()(syntax::global::Function const& node) -> Str {
            return std::format(
