...
namespace tlc::lex {
  ...
  auto Lexer::addToken(token::EToken tokenType) -> void {
  ...
  auto Lexer::operator()(const fs::path &filepath) -> Vec<Token> {
    reset();
  ...
  auto Lexer::operator()(std::istringstream ss) -> Vec<Token> {
    reset();
  ...
  auto Lexer::lexNumericLiteral() -> bool {
    ...
    if (lexed && m_lastTokenType == Invalid) {
  ...
  auto Lexer::lexOperator() -> bool {
    ...
    if (Token::isValidOperator(m_currentToken + m_currentChar + peek())) {
      skip(2);
      addToken(MaybeOperator); // valid 3-char operator
      return true;
    }

    if (Token::isValidOperator(m_currentToken + m_currentChar)) {
      skip();
      addToken(MaybeOperator); // valid 2-char operator
      return true;
    }

    addToken(MaybeOperator);
    if (m_lastTokenType == Invalid) {
